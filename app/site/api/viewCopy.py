...
class site:
    ...
    @renderer_classes((JSONRenderer))
    @api_view(['GET'])
    async def scarlet(request: Request):
        scarletBase = {
            "META": {
                "repoName": "Paul's Decrypt.day Repo",
                "repoIcon": "https://psli.space/static/img/siteLogo.png"
            },
            "Games": []
        }
        
        if request.method == "GET":
            # Get a request to the API via aiohttp
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    gamesHTML = games["solution"]["response"]
                    soup = BeautifulSoup(gamesHTML, features="html.parser")
                    ...
                    if resp.status == 200:
                        for game in games:
                            ...
                            scarletBase["Games"].append(totalData)
                        return JsonResponse(scarletBase, safe=False)
                    else:
                        return JsonResponse({"error": "Failed to fetch games!"}, safe=False)
