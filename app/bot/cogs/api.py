...
class interact(commands.Cog):
    ...
    async def filetree(
        ...
        paths = fileManager.DisplayablePath.make_tree(
            Path(os.path.join(Dirs.FILES, folder))
        )
        for path in paths:
        ...
        await interaction.response.send_message(
            embed=embed.createEmbed(
                title=f"File Tree of `./{folder.replace(PARENT_DIR, '')}`",
                description=tree,
                timestampFooter=True,
            ),
            ephemeral=True,
        ...
        await interaction.response.send_message(
            embed=embed.createEmbed(
                ...
                description=dedent(
                    f"""
                    Access your file @
                    {requote_uri(f"{ENV['site']['siteAddress']}{filePath.replace(PARENT_DIR, '')}")}
                    """
                ),
    ...
    async def delete(
        ...
        try:
            os.remove(os.path.join(Dirs.FILES, filepath))
        except FileNotFoundError:
            os.remove(os.path.join(Dirs.FILES, filepath[1:]))
        await interaction.response.send_message(
            embed=embed.createEmbed(
                title=f"`{filepath.split(os.sep)[-1]}` deleted",
                description=dedent(
                    f"""
                    `{filepath.split(os.sep)[-1]}` has been deleted from `{os.sep.join(filepath.split(os.sep)[:-1]).replace(Dirs.FILES, '') if '' != os.sep.join(filepath.split(os.sep)[:-1]).replace(Dirs.FILES, '') else '.'}`
                    """
                ),
            ),
            ephemeral=True,
    ...
    async def mkdir(
        ...
        interaction: Interaction,
        name: str,
        folder: str = SlashOption(name="folder", default=Dirs.FILES, description="Which folder would you want to create the folder in?"),
    ):
        os.mkdir(os.path.join(Dirs.FILES, folder, name))
        await interaction.response.send_message(
            embed=embed.createEmbed(
                title=f"`{name}` created",
                description=dedent(
                    f"""
                    Access your folder @ {requote_uri(f"{ENV['site']['siteAddress']}{(os.path.join(folder, name).replace(PARENT_DIR, '') if os.path.join(folder, name).replace(PARENT_DIR, '')[0] == '/' else '/' + os.path.join(folder, name).replace(PARENT_DIR, ''))}")}
                    """
                ),
            ),
            ephemeral=True,
    ...
    async def rmdir(
        ...
        shutil.rmtree(os.path.join(Dirs.FILES, folder), ignore_errors=True)
        await interaction.response.send_message(
            embed=embed.createEmbed(
                title=f"`{folder.split(os.sep)[-1]}` deleted",
                description=dedent(
                    f"""
                    Your folder has been deleted in: ```./{os.path.join(folder).replace(PARENT_DIR, '')}```
                    """
                ),
            ),
            ephemeral=True,
    ...
    async def retrieve(
        ...
        path = (
            os.path.join(Dirs.FILES, filepath)
            if os.path.isfile(os.path.join(Dirs.FILES, filepath))
            else os.path.join(Dirs.FILES, filepath[1:])
        )
        ...
        with open(path, "rb") as f:
            await interaction.response.send_message(
                embed=embed.createEmbed(
                    ...
                    description=dedent(
                        f"""
                        Access your file @ {requote_uri(f"{ENV['site']['siteAddress']}{path.replace(PARENT_DIR, '')}")}
                    ...
                    image=requote_uri(
                        f"{ENV['site']['siteAddress']}{path.replace(PARENT_DIR, '')}"
                    )
