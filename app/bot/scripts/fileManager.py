...
def structure(dictReturn: bool = True, safe: bool = False):
    context = {}
    for root, foldersWithinSubFolder, files in os.walk(Dirs.FILES, topdown=True):
        for file in files:
            if os.path.isfile(os.path.join(root, file)):
                filePath = f'/files/{os.path.join(root, file).replace(Dirs.FILES, "")[1:]}'
                relPath = f'./{os.path.split(os.path.join(root, file))[0].replace(Dirs.FILES, "")[1:]}' if (len(os.path.split(os.path.join(root, file))[0].replace(Dirs.FILES, "")[1:]) != 0) else (".")
                ...
                context.update(fileInfo) if safe == False else (context.update(fileInfo) if os.path.join(root, file) not in [os.path.join(Dirs.FILES, dnd[2:]) for dnd in DO_NOT_DELETE] else None)

    return context

def folder(ignoreBase: bool = False):
    if not ignoreBase:
        return {x[0].replace(Dirs.FILES, "."): x[0].replace(Dirs.FILES, "")[1:] for x in os.walk(Dirs.FILES)}
    else:
        returnDict = {}
        for x in os.walk(Dirs.FILES):
            if x[0] != Dirs.FILES:
                returnDict.update({x[0].replace(Dirs.FILES, "."): x[0].replace(Dirs.FILES, "")[1:]})
        return returnDict
